...
Natural Earth at 1:110m, which is already in this repository — no network, no
key, no tile policy, and it is the application drawing its own data with its own
projection rather than borrowing someone's basemap.

110m and not 10m on purpose: this is a locator, not the product. The coarse set
is about five thousand vertices for the whole world, which a Tk canvas redraws
without complaint; the detailed one is two orders of magnitude more and would
make dragging stutter for detail nobody can see at this size.

**A missing dataset is not an error.** The locator degrades to a graticule and
the application still runs — someone who cloned without the datasets should get
a working window, not a traceback.
"""
...
def load(root: Path | None = None) -> Outline:
    """Read the outline, or return an empty one.
